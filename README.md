...
常用参数：`--gpu`、**`--mcts-sims`**（MCTS 模拟次数上限，默认 `192`，可按机器与棋力需求调大）、**`--mcts-max-seconds`**（墙钟秒数上限，与模拟次数先到先停；不设则仅按次数）、**`--c-puct`**（PUCT 系数，默认 `1.5`）。对弈界面在 MCTS 落子后会在状态栏附带上一轮统计（玩法数、耗时、网络展开次数、根访问合计、停止原因）。
