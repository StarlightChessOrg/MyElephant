...
def _cell_display_char(fench):
    """每个格子一个全角宽字符，避免与半角框线混排时终端错位。"""
    if not fench:
        return u"\u3000"  # 全角空格
    return _fench_to_txt_name(fench)
...
class BaseChessBoard(object) :
    ...
    def dump_board(self):
        """
        文本棋盘：每行 9 个全角格（棋子或 U+3000），与行号、列标对齐，
        不再使用半角 ┌─┐ 模板，避免 CJK 终端里全角棋子挤乱半角网格。
        """
        lines = []
        for y in range(9, -1, -1):
            rank_label = str(9 - y)
            row = "".join(_cell_display_char(self._board[y][x]) for x in range(9))
            lines.append(rank_label + " " + row)
            if y == 5:
                lines.append("  " + u"\u3000" * 9)
        col_line = "  " + "".join(str(i).center(2) for i in range(9))
